...
// 🧭 Títulos (Headings) alinhados à esquerda sem recuo de parágrafo
  ...
  set table(
    stroke: (x, y) => if y == 0 {
      (top: 0.8pt + primary-color, bottom: 0.8pt + primary-color)
    } else {
      (bottom: 0.5pt + primary-color.transparentize(50%))
    },
    fill: (x, y) => if y == 0 {
      primary-color.transparentize(70%)
    } else if calc.even(y) {
      primary-color.transparentize(90%)
    }
  )
